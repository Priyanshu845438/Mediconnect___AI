...
import { CalendarDaysIcon, ChatBubbleLeftEllipsisIcon } from '../../components/icons/SolidIcons'; // Added ChatBubbleLeftEllipsisIcon
  ...
  return (
    <div className="space-y-8"> {/* Increased space */}
      <header className="bg-white shadow-xl rounded-xl p-6 sm:p-8"> {/* Increased padding and shadow */}
        <div className="flex items-center">
            <div className="p-3 bg-primary/10 rounded-full mr-4">
                <CalendarDaysIcon className="w-10 h-10 text-primary" /> {/* Larger icon */}
            </div>
            <div>
                <h1 className="text-3xl sm:text-4xl font-bold text-primary">My Bookings</h1> {/* Larger title */}
      ...
      {sortedAppointments.length > 0 ? (
        <div className="bg-white shadow-xl rounded-xl p-0 overflow-hidden"> {/* Increased shadow */}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedAppointments.map((app) => (
                  <tr key={app.id} className="hover:bg-teal-50/50 transition-colors duration-150"> {/* Softer hover */}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${app.paymentStatus === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}> {/* Increased padding in badge */}
                        {app.paymentStatus}
                      </span>
                    </td>
        ...
        <div className="bg-white shadow-xl rounded-xl p-10 text-center"> {/* Increased shadow and padding */}
          <ChatBubbleLeftEllipsisIcon className="w-20 h-20 text-gray-300 mx-auto mb-6" /> {/* Changed icon */}
